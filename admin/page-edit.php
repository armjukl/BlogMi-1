...
if (isset($_POST['_IS_POST_BACK_'])) {
  ...
  if ($page_title == '') {
    ...
    if ($page_file == '') {
      ...
      foreach ($file_names as $file_name) {
        $file_path = '../files/pages/data/'.$file_name.'.dat';
    ...
    $index_file = '../files/pages/index/'.$page_state.'.php';
    require $index_file;
  ...
  $file_path = '../files/pages/data/'.$_GET['file'].'.dat';
  $data = unserialize(file_get_contents($file_path));
  $page_file        = $data['file'];
  $page_path        = $data['path'];
...
<form action="<?php echo $_SERVER['REQUEST_URI']; ?>" method="post">
  ...
  <div style="margin-bottom:20px;">
    ...
    <textarea id="elm1" name="content" style="width: 860px; height: 450px; display: none; "><?php echo htmlspecialchars($page_content); ?></textarea>
  </div>
  <div style="margin-bottom:20px;">
